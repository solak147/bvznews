export default defineNuxtPlugin(() => {
  ...
  const { fileUrl } = runtimeConfig.public
  ...
  return {
    provide: {
        ...
        const { data, pending, error, refresh } = await useFetch(url, {
          ...
          onResponse({ request, response, options }) {
            if (response._data.data) {
              // -1已捕捉錯誤
              if (response._data.data.code === -1) {
                showDialog({
                  message: response._data.data.msg,
                  theme: 'round-button'
                })
                return
              }
              ...
              // -3未捕捉錯誤
              if (response._data.data.code === -1 || response._data.data.code === -3) {
                console.log(response._data.data.msg)
                navigateTo('/error')
              }
            }
      ...
      upload: async (file, param = 'work') => {
        ...
        await useFetch(`${fileUrl}/file/upload/${param}`, {
          method: 'post',
          body: formData,
          onRequest({ options }) {
            const token = useCookie('jwt-token')
